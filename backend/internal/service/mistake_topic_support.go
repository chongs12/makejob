...
// resolveMistakeTopicByTag 根据错因标签定位专题卡片。
func resolveMistakeTopicByTag(tag string) (*MistakeTopicCard, bool) {
...
// resolveMistakeTopicCodeByTag 根据错因标签返回专题编码。
func resolveMistakeTopicCodeByTag(tag string) string {
	if topic, ok := resolveMistakeTopicByTag(tag); ok {
		return topic.Code
	}
	return ""
}

// listMistakeTopicsByCodes 根据编码列表返回去重后的专题卡片集合。
func listMistakeTopicsByCodes(codes []string) []MistakeTopicCard {
	result := make([]MistakeTopicCard, 0, len(codes))
	...
	for _, code := range codes {
		...
		if _, exists := seen[topic.Code]; exists {
			continue
		}
		seen[topic.Code] = struct{}{}
