...
> ⚠️ **路径迁移说明（2026-07-09）**：本报告基于单体 `backend/` 目录分析（见下文「仅基于 backend 目录」），该单体已归档至 `docs/backend/`。当前 AI 面试能力分布在微服务 `app/interview/`、`app/ai_gateway/`、`app/realtime/`。作为求职话术的概念框架仍可参考，但代码路径均已迁移。
