...
- **后端**：`app/` 下 15 个微服务（Kratos DDD 四层）+ 1 个 BFF Gateway。Gateway 是唯一 HTTP 入口（`:8082`），对外暴露 `/api/v1/*`，内部通过 gRPC 调用各服务。
...
| 依赖 | 用途 | 默认地址 |
|------|------|---------|
| PostgreSQL | 各服务数据存储 | `localhost:5434`（user=postgres / pwd=password / db=makejob） |
| Redis | User 服务 Token 黑名单 / 缓存 | `localhost:6384` |
...
15 个服务及其端口（gRPC 为主；Gateway/Realtime 另有 HTTP/WS）：

| 服务 | gRPC | HTTP/WS | 服务 | gRPC | HTTP/WS |
|------|------|---------|------|------|---------|
| Gateway | - | :8082 | Community | :9009 | :8009 |
| User | :9101 | :8101 | LearningArchive | :9010 | :8010 |
| Membership | :9002 | :8002 | AI Gateway | :9011 | :8011 |
| Question | :9003 | :8003 | RAG | :9012 | :8012 |
| Interview | :9004 | :8004 | CodeRunner | :9013 | - |
| Realtime | :9005 | :8085(WS) | Admin（BFF） | :9014 | :8014 |
| Growth | :9006 | :8006 | Plan | :9007 | :8007 |
| Companion | :9008 | :8008 | | | |

> 各服务同时开 gRPC 与 HTTP 两个监听（CodeRunner 仅 gRPC）；端口以各 `app/<svc>/configs/config.yaml` 为准。
