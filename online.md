...
- 2026-05-23：已按本文档修正仓库内实时面试接入约束，后端统一要求使用 `volcengine.realtime.app_id + access_token` 建连，并固定使用文档规定的 `X-Api-App-Key=PlgvMymc7f3tQnJ6`。
- 2026-05-23：已补上实时面试模式识别，实时面试记录不会再因为配置开关变化被误回退到旧的 HTTP/TTS 问答链路。
- 2026-05-23：已补上面试页 Live2D 本地资源兜底；当后台数据库未配置模型记录时，前台会回退扫描 `live2d-src` 目录并返回可用模型。
- 2026-05-23：已按文档修正实时首轮与收音收口事件：首题改为使用 `501 ChatTextQuery` 触发模型真正生成问题，前端结束录音时后端会显式发送 `400 EndASR`，避免会话卡在“正在整理回答”。
- 2026-05-23：已继续修正浏览器录音上传链路：前端录音上下文会显式 `resume`，采集音频会重采样到文档要求的 `16k PCM` 并按约 20ms 分包发送；后端若一整轮未收到任何用户音频块，会直接提示重新录音而不是继续卡在 ASR 等待态。
- 2026-05-23：已继续修正前端录音收口时序：停止录音后不再瞬时冲刷整段排队音频，而是等待队列中的 `20ms PCM` 音频帧按节奏发送完毕后再补发 `audio_end`，避免火山实时 ASR 因收到突发大块音频而不返回 `450/451/459`。
- 2026-05-23：当前仍需在实际运行环境提供有效的火山实时语音 `app_id` 与 `access_token`，否则后端会明确报出配置缺失，不再静默回退冒充成功调用实时模型。
